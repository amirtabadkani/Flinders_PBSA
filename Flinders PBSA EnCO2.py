...
st.set_page_config(page_title='Flinders PBSA - Adelaide', layout='wide')
...
st.title(":red[Flinders PBSA] - Building Performance Analytical Dashboard")
...
with st.sidebar:
    ...
    Shade_type = st.select_slider('Shade Type:', options = ['No Shade','1m V/H Shade','Screen'], value = 'No Shade', key = 'shadetype')
...
with st.container():
    ...
    with cols[0]:
        ...
        gauge = {'bar':{'color':'white', 'line':{'width':3}},'bordercolor':'darkgray','bgcolor':'lightgray', 'threshold':{'thickness':1,'value':REF_EUI}, 'axis':{'dtick':5,'range':[1,50]}},
    ...
    with cols[2]:
        ...
        gauge = {'bar':{'color':'white', 'line':{'width':3}},'bordercolor':'darkgray','bgcolor':'lightgray', 'threshold':{'thickness':1,'value':REF_Cost/1000}, 'axis':{'dtick':2,'range':[1,20]}},
        number = {'font':{'family':'Arial'}},
        domain = {'x': [0, 1], 'y': [0, 1]},
    ...
    with cols[3]:
        ...
        gauge = {'bar':{'color':'white', 'line':{'width':3}},'bordercolor':'darkgray','bgcolor':'lightgray', 'threshold':{'thickness':1,'value':REF_DA}, 'axis':{'dtick':5,'range':[0,100]}},
        number = {'font':{'family':'Arial'}},
    ...
    with cols[4]:
        ...
        gauge = {'bar':{'color':'white', 'line':{'width':3}},'bordercolor':'darkgray','bgcolor':'lightgray', 'threshold':{'thickness':1,'value':REF_glare}, 'axis':{'dtick':2,'range':[0,15]}},
        number = {'font':{'family':'Arial'}},
    ...
    def loadImages():
        ...
        ref = Image.open(r'C:\Users\atabadkani\StreamlitApps\FlindersEnCO2\data\images\REF.png')
    ...
    columns = ['WWR-NS', 'WWR-EW', 'ShadeType', 
       'SHGC','VLT', 'ExWall', 'Ventilation','EUI (kWh/m2)', 'EUI Saved(-)Wasted(+)',
    ...
    chep_pcm = px.parallel_coordinates(CHEP_ML, columns, color="EUI (kWh/m2)",
                                       ...
                                       color_continuous_midpoint=7, height = 800, width = 2000)
    
    chep_pcm.update_layout(coloraxis=dict(cmin=min_eui), coloraxis_showscale=True,font=dict(size=15),margin_autoexpand= True)
...
feature_energy_cols =  CHEP_ridge[['WWR-NS', 'WWR-EW', 'ShadeType', 'SHGC', 'ExWall', 'Ventilation']].columns

X = pd.DataFrame(CHEP_ridge[['WWR-NS', 'WWR-EW', 'ShadeType', 'SHGC', 'ExWall','Ventilation']], columns = feature_energy_cols)
...
with cols[1]:
    ...
    if new_shade == 0:
        ...
        st.markdown("**1m Horizontal shade to N/S spaces, and Vertical shades to E/W spaces**")
    elif new_shade == 2:
    ...
    new_vals_energy = np.array([new_WWR_NS,new_WWR_EW,new_shade,new_SHGC,new_exwall,new_ventilation]).reshape(1,6)
