...
flinders_en = pd.read_csv('./data/Flinders.csv')
flinders_ML = pd.read_csv('./data/Flinders_ML.csv')


# flinders_en = pd.read_csv(r'C:\Users\atabadkani\StreamlitApps\FlindersEnCO2\data\Flinders.csv')

# flinders_ML = pd.read_csv(r'C:\Users\atabadkani\StreamlitApps\FlindersEnCO2\data\Flinders_ML.csv')
...
flinders_en['Energy Cost (AU$/yr)'] = flinders_en['EUI (kWh/m2)']*Floor_area*en_price
flinders_ML['Energy Cost (AU$/yr)'] = flinders_ML['EUI (kWh/m2)']*Floor_area*en_price

with st.sidebar:
    st.image('./data/Stantec_Logo.png',output_format='PNG')
...
flinders_en_RESULT = flinders_en[flinders_en['WWR-NS'].isin([WWR_NS]) & flinders_en['WWR-EW'].isin([WWR_EW]) & flinders_en['ShadeType'].isin([Shade_type]) & flinders_en['SHGC'].isin([SHGC]) & flinders_en['U-value'].isin([Uvalue]) & flinders_en['ExWall'].isin([exwall]) & flinders_en['Ventilation'].isin([ventilation])]

with st.container():
    
    st.subheader(f'Design Selection Summary - Iteration {flinders_en_RESULT.index[0]} ')

    def get_metrics_EUI():
    
        flinders_en_RESULT = flinders_en[flinders_en['WWR-NS'].isin([WWR_NS]) & flinders_en['WWR-EW'].isin([WWR_EW]) & flinders_en['ShadeType'].isin([Shade_type]) & flinders_en['SHGC'].isin([SHGC]) & flinders_en['U-value'].isin([Uvalue]) & flinders_en['ExWall'].isin([exwall]) & flinders_en['Ventilation'].isin([ventilation])]
        
        EUI_METRIC = flinders_en_RESULT['EUI (kWh/m2)']
        ELEC_P = flinders_en_RESULT['ELECp (W/m2)']
        AVE_DA =  flinders_en_RESULT['Daylight Autonomy']
        AVE_UDI = flinders_en_RESULT['Excessive Daylight']
        ENERGY = flinders_en_RESULT['Energy Cost (AU$/yr)']
        Space_NE = flinders_en_RESULT['OT26% - L5-Student-Com-NE']
        Space_SW = flinders_en_RESULT['OT26% - L5-Studio-SW']
        Space_SE= flinders_en_RESULT['OT26% - L5-Studio-SE']
        Space_NW = flinders_en_RESULT['OT26% - L5-Studio-NW']
        image = flinders_en_RESULT['img']
        EUI_REF = flinders_en_RESULT['EUI Saved(-)Wasted(+)']
        ELEC_REF = flinders_en_RESULT['REF_ELECp']
        DA_REF = flinders_en_RESULT['REF_DA']
        UDI_REF = flinders_en_RESULT['REF_ExcDA']
        # ventilation = flinders_en_RESULT['Ventilation']
    ...
    st.markdown(":red[**_Peak Electricity Load (ELECp):_**] This is the maximum electricity demand per unit area of the building, measured in W/m2. High peak loads can lead to higher utility costs, especially in areas where utilities charge are based on peak demand.")
    ...
    with cols[1]:
        ...
        gauge = {'bar':{'color':'white', 'line':{'width':3}},'bordercolor':'darkgray','bgcolor':'lightcyan', 'threshold':{'thickness':1,'value':REF_ELECp,'line': {'color': "red", 'width': 4}}, 'axis':{'dtick':1,'range':(flinders_en['ELECp (W/m2)'].min(),flinders_en['ELECp (W/m2)'].max())}},
    ...
    # max_eui = flinders_ML['EUI (kWh/m2)'].max()
    # min_eui = flinders_ML['EUI (kWh/m2)'].min()
    # cols = st.columns([0.1,2,0.1])
    # with cols[0]:
    #     ""
    # with cols[1]:

    #     flinders_pcm = px.parallel_coordinates(flinders_ML, columns, color="EUI (kWh/m2)",
    #                                     color_continuous_scale=px.colors.cyclical.Edge,
    #                                     range_color=[min_eui, max_eui],
    #                                     color_continuous_midpoint=7, height = 800, width = 2500)
        
    #     flinders_pcm.update_layout(coloraxis=dict(cmin=min_eui), coloraxis_showscale=True,font=dict(size=17),margin_autoexpand= True,autosize=True,xaxis=dict(domain=[0, 1]))
        
    #     st.plotly_chart(flinders_pcm, use_container_width=True)
...
# st.write(flinders_en.columns)
st.subheader("**Design Explorer**")

color_metric = 'EUI (kWh/m2)'  # <- numeric recommended for continuous coloring

# Some curated palettes to choose from

scale_options = {
    "Plasma (purple → pink → yellow)": px.colors.sequential.Plasma,
    "Viridis (dark blue → green → yellow)": px.colors.sequential.Viridis,
    "Turbo (blue → green → yellow → red)": px.colors.sequential.Turbo,
    "Cividis (blue → green → yellow, colorblind-friendly)": px.colors.sequential.Cividis,
    "Edge (cyclical: blue → purple → green)": px.colors.cyclical.Edge,
    "IceFire (cyclical: blue → white → orange)": px.colors.cyclical.IceFire,
    "Twilight (cyclical: purple → pink → teal)": px.colors.cyclical.Twilight,
    "Bluered (diverging: blue ↔ red)": px.colors.diverging.RdBu,
    "Spectral (diverging: blue → green → yellow → red)": px.colors.diverging.Spectral,
}
...
with cols[0]:
    selected_scale_name = st.selectbox(
        ...
        index=list(scale_options.keys()).index("Edge (cyclical: blue → purple → green)"),
    )

selected_scale = scale_options[selected_scale_name]

# Ensure the color field is numeric; factorize if needed
color_series = flinders_en[color_metric]

if not np.issubdtype(color_series.dtype, np.number):
    # Factorize to numeric if a non-numeric column was selected
    flinders_en = flinders_en.copy()
    flinders_en["_color_num"] = pd.factorize(color_series)[0]
    color_field = "_color_num"
else:
    color_field = color_metric

columns = ['WWR-EW', 'WWR-NS', 'ShadeType', 
'SHGC','VLT', 'U-value','ExWall', 'Ventilation','EUI (kWh/m2)', 'EUI Saved(-)Wasted(+)','ELECp (W/m2)',
'Daylight Autonomy', 'Excessive Daylight', 'Energy Cost (AU$/yr)']


flinders_pcm = px.parallel_coordinates(flinders_en, columns, 
                                labels={"ShadeType": "ShadeType", "ExWall":"ExWall"},
                                color=color_field,
                                color_continuous_scale=selected_scale,  
                                color_continuous_midpoint=2, height = 800)

flinders_pcm.update_layout(coloraxis_showscale=False,
                        font=dict(size= 16, color="black"),   # make labels black
                        paper_bgcolor="white",
                        plot_bgcolor="white",
...
with st.expander('Statistical Building Performance Correlations (Box Plots)'):
    ...
    flinders_bx_01 = px.box(flinders_en, flinders_en["WWR-NS"], flinders_en["EUI Saved(-)Wasted(+)"], "WWR-NS", notched = True)
    flinders_bx_02 = px.box(flinders_en, flinders_en["WWR-EW"], flinders_en["EUI Saved(-)Wasted(+)"], "WWR-EW", notched = True)
    flinders_bx_03 = px.box(flinders_en, flinders_en["ShadeType"], flinders_en["EUI Saved(-)Wasted(+)"], "ShadeType", notched = True)
    flinders_bx_04 = px.box(flinders_en, flinders_en["SHGC"], flinders_en["EUI Saved(-)Wasted(+)"], "SHGC", notched = True)
    flinders_bx_05 = px.box(flinders_en, flinders_en["ExWall"], flinders_en["EUI Saved(-)Wasted(+)"], "ExWall", notched = True)
    flinders_bx_06 = px.box(flinders_en, flinders_en["Ventilation"], flinders_en["EUI Saved(-)Wasted(+)"], "Ventilation", notched = True)

    cols = st.columns(6)
    
    with cols[0]:
        st.plotly_chart(flinders_bx_01, use_container_width=True)
    with cols[1]:
        st.plotly_chart(flinders_bx_02, use_container_width=True)
    with cols[2]:
        st.plotly_chart(flinders_bx_03, use_container_width=True)
    with cols[3]:
        st.plotly_chart(flinders_bx_04, use_container_width=True)
    with cols[4]:
        st.plotly_chart(flinders_bx_05, use_container_width=True)
    with cols[5]:
        st.plotly_chart(flinders_bx_06, use_container_width=True)

    flinders_bx_07 = px.box(flinders_en, flinders_en["WWR-NS"], flinders_en["Daylight Autonomy"], "WWR-NS", notched = True)
    flinders_bx_08 = px.box(flinders_en, flinders_en["WWR-EW"], flinders_en["Daylight Autonomy"], "WWR-EW", notched = True)
    flinders_bx_09 = px.box(flinders_en, flinders_en["ShadeType"], flinders_en['Daylight Autonomy'], "ShadeType",  notched = True)
    flinders_bx_10 = px.box(flinders_en, flinders_en["VLT"], flinders_en['Daylight Autonomy'], "VLT",  notched = True)
    
    cols = st.columns(4)
    
    with cols[0]:
        st.plotly_chart(flinders_bx_07, use_container_width=True)
    with cols[1]:
        st.plotly_chart(flinders_bx_08, use_container_width=True)
    with cols[2]:
        st.plotly_chart(flinders_bx_09, use_container_width=True)
    with cols[3]:
        st.plotly_chart(flinders_bx_10, use_container_width=True)

    flinders_bx_13 = px.box(flinders_en, flinders_en["WWR-NS"], flinders_en["Excessive Daylight"], "WWR-NS", notched = True)
    flinders_bx_14 = px.box(flinders_en, flinders_en["WWR-EW"], flinders_en["Excessive Daylight"], "WWR-EW", notched = True)
    flinders_bx_15 = px.box(flinders_en, flinders_en["ShadeType"], flinders_en['Excessive Daylight'], "ShadeType",  notched = True)
    flinders_bx_16 = px.box(flinders_en, flinders_en["VLT"], flinders_en['Excessive Daylight'], "VLT",  notched = True)
    
    cols = st.columns(4)
    
    with cols[0]:
        st.plotly_chart(flinders_bx_13, use_container_width=True)
    with cols[1]:
        st.plotly_chart(flinders_bx_14, use_container_width=True)
    with cols[2]:
        st.plotly_chart(flinders_bx_15, use_container_width=True)
    with cols[3]:
        st.plotly_chart(flinders_bx_16, use_container_width=True)
...
flinders_ridge = flinders_ML.drop(['img', 'REF_ELECp','REF_DA','REF_ExcDA'], axis=1)

with st.expander('Statistical Building Performance Correlations (Heat Map)'):

    flinders_CORR_HTM = px.imshow(round(flinders_ridge.corr(),2),text_auto=True,color_continuous_scale='thermal',  width = 1000, height = 1000)
    flinders_CORR_HTM.update_traces(textfont_size=15)
    
    st.plotly_chart(flinders_CORR_HTM, use_container_width=True)
...
feature_energy_cols =  flinders_ridge[['WWR-NS', 'WWR-EW', 'ShadeType', 'SHGC','U-value', 'ExWall', 'Ventilation']].columns

X = pd.DataFrame(flinders_ridge[['WWR-NS', 'WWR-EW', 'ShadeType', 'SHGC', 'U-value', 'ExWall','Ventilation']], columns = feature_energy_cols)

y = flinders_ridge[['EUI (kWh/m2)', 'ELECp (W/m2)', 'OT26% - L5-Student-Com-NE', 'OT26% - L5-Studio-SW','OT26% - L5-Studio-SE', 'OT26% - L5-Studio-NW','EUI Saved(-)Wasted(+)','Energy Cost (AU$/yr)']]
...
flinders_en_cdf = pd.DataFrame(np.transpose(ridge_en.coef_),X.columns,columns=[['EUI (kWh/m2)', 'ELECp (W/m2)', 'OT26% - L5-Student-Com-NE', 'OT26% - L5-Studio-SW','OT26% - L5-Studio-SE', 'OT26% - L5-Studio-NW','EUI Saved(-)Wasted(+)','Energy Cost (AU$/yr)']])
